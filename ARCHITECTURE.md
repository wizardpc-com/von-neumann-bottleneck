...
- `src/hardware_foundations/` owns the registry-driven level-map view from tutorial through CPU, Half Adder and later construction interactions, authoritative GraphEdit export, fixed external Test Benches, procedural symbols, port/segment/waypoint wiring and context deletion, transactional undo/redo, Shift multi-selection, selected-subgraph copy/paste, automatic layouts, live port feedback, floating Mission/Test Bench windows, parallel exact-path playback, and encapsulation presentation.
