...
`src/ui/prototype_hub.tscn` is the project entry. Ordinary Game starts at the hub, with the shared prerequisite tree as its primary entry and five chapter cards as alternate navigation. Continue selects the most recently visited available task on that tree; navigation never grants progress. The original Hardware Foundations arithmetic/storage branches converge at CPU and the LOAD/STORE bridge. Chapters 1–4 cover system costs, locality, scheduling and data layout, with optional application branches. `CircuitGraphEdit` retains free building, branched wires and transactional edits. Mission remains movable and reopenable; Hint uses a separate read-only board with individually confirmed H2/H3. Shared presentation adds distinct scalar/bus strokes, module headings, net inspection and view-only Focus. Mission exposes direct specification sections; success exposes explicit sealing and next-capability previews. See [construction experience](docs/status/construction-experience.md).

The original chapters share `InstrumentTheme`, `TechnicalBackdrop` and bundled Noto Sans SC typography. `SignalNotation` supplies explicit width badges, stable width colors and bit-cell examples shared by terminals, Test Bench and Mission. One-bit cables are thin solid strokes with round pins; wider cables are centered ribbons with square sockets. The graph, drafts, hover/flow feedback and component leads share that notation without changing connection geometry. Compatibility enumeration and hover previews are read-only; a rejected release reports the diagnostic once. Wire replay and bus-value badges resolve current connection geometry while panning, zooming or moving components. Reduced motion uses static feedback without changing the presentation clock or simulation. Terminal bodies show decimal values and visible bits; signal names sit above them. Existing port geometry, simulation values and player wire colors are preserved. `SignalLevelButton` preserves the existing CheckButton input contract while drawing compact low/high symbols. Palette thumbnails reuse the canvas schematic and module renderers; placement ghosts retain the full GraphNode geometry. `TerminologyHandbook.LESSON_TERMS` derives subject availability from existing original progression, including each playable Mission's required specifications. Its progressive diagrams show one example at a time. Layout and batch illustrations use the real `LayoutRecipe` address mapping; illustration controls never modify player designs. It adds no persistent unlock authority or save fields. See [visual and learning polish](docs/status/visual-learning-polish.md).

## Removed eight-task fork

The eight-task comparison runtime, autoload, exclusive translations and tests were
removed on 2026-09-09 by explicit user request. Existing comparison save files are
left untouched and never grant original campaign completion. Historical design
and evidence remain in Git and marked historical docs.
...
fresh official pass + unchanged topology signature
  ...
  -> PlayerContentState reusable component in the Game library
...
The five Chapter 1 investigations require their authored program signatures for controlled comparisons. The optional `read_once` task accepts valid player-written programs against its official cases and request/cost constraints; `two_orders` checks the two authored workload signatures and each order’s parts/budget. Before the first final diagnosis, the authored program and default machine parts remain fixed alongside the hidden breakdown.
...
- `src/playtest/playtest_data.gd` owns one anonymous session identity, an immediately flushed versioned JSONL event stream, interrupted-session recovery, derived level summaries, a versioned single-file JSON export, and the raw-session/workbench portion of the explicit `--reset-local-test-state` developer cleanup. It observes existing controller boundaries only: callers ignore write/export failures, and the service never supplies simulation inputs, completion evidence, progression state, scores, Trace data, or playback state. Every event carries Game/Test mode; full Program source, Notebook text, machine identity, and unrelated personal data are excluded. Global save cleanup is independently owned by `GlobalSave`; prior exports remain preserved.
- `src/save/global_save.gd` owns the schema-2 Game-only recovery index (retaining the filename) at `user://savegame_v1.json`, guarded temporary/backup replacement, chapter completion/gate snapshots, and Continue/New Game lifecycle. It never treats telemetry or completion flags as reusable-component authority: every primary reusable is reconstructed from an independent Game workbench whose source signature and current official verifier both match; generated wrappers are recreated from the current recipe and verified source signature. Missing or stale provenance invalidates only registered dependents. Receipts, Trace state, editor history, debug inputs, window geometry, and Test progression are excluded. Signature revision 2 uses textual ID order and canonical integral JSON properties. Legacy conversion retains content-addressed originals and restores only reverified provenance; see [ADR 0020](docs/decisions/0020-stable-signatures-and-verified-legacy-recovery.md).
- Playtest event schema 2 (separate from the save schema) adds visit/run/case identities, source classification and monotonic foreground/background/feedback checkpoints. Old schema 1 retains legacy uncertainty. `src/playtest/playtest_moments.gd` provides voluntary in-play feedback and export; `scripts/report-playtests.py` merges local exports with source separation, deduplication and incomplete visits. Neither is progression authority.
...
- `src/content/` owns explicit campaign branch/level descriptors, deterministic registry validation/order/dependencies, bounded prologue content packs, reward recipes, and `PlayerContentState`. Game state is reconstituted by the save service only through verified workbench provenance; Test state remains session-only. It describes arrangements of supported mechanics but does not execute electrical behavior. See [`docs/architecture/content-system.md`](docs/architecture/content-system.md).
...
- `src/system_lab/` owns Chapter 1's separate 8-bit CPU/RAM/Bus specifications, six typed routes, bounded program parser, sequential cost model, deterministic Trace/receipt/diagnosis, seven-task catalog (five investigations plus two optional applications), isolated Game/Test chapter state, graphical chapter map, system desktop, player-colored route rendering, component-native feedback, exact-segment flow-band/value-badge overlay, an editable presentation Clock Period, controlled Before → After evidence, gated diagnostic breakdown, and completion handoff to its own map. It deliberately has no Cache or circuit-gate evaluator.
- Chapter 1's Bus surface groups eight visible bit cells by the selected bandwidth. Active values, widths and group counts come from the authoritative transfer event; playback progress only selects the illustrated group. Read/write/request overlays select their corresponding typed routes. The desktop retains the displayed event/progress so paused zoom, pan and device movement redraw the same event without advancing simulation. Changing a part clears current result cards while preserving comparison History.
- The original hardware, system and locality floating desktops synchronize focused panels' sibling order with their drawing order: Godot Control input picking does not follow `z_index`. Windows remain freely movable and independently closable.
...
- `tests/test_global_save.gd` verifies partial prologue and branch resume, source-signature/workbench reconciliation, generated-wrapper lineage, Chapter 1/2 gates, derived Notebook unlocks, selective invalidation, backup recovery, corrupt/future schemas, confirmed New Game boundaries, Game/Test isolation, and telemetry/workbench independence.
...
`RemoteFeedback` observes future v2 events after separate consent; bounded asynchronous outbox and explicit opinions never affect results or saves. The loopback receiver is HTTP + SQLite, with durable acknowledgements, idempotence and scoped deletion. The candidate has no configured endpoint. [Local setup and privacy boundary](server/README.md). `UiMotion` provides 120 ms presentation-only panel fades. Reduced motion also makes hardware trace feedback static; simulation results, playback timing and input acceptance remain unchanged.
