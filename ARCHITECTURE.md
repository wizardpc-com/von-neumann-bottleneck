...
- `src/playtest/playtest_data.gd` owns one anonymous session identity, an immediately flushed versioned JSONL event stream, interrupted-session recovery, derived level summaries, and a versioned single-file JSON export. It observes existing controller boundaries only: callers ignore write/export failures, and the service never supplies simulation inputs, completion evidence, progression state, scores, Trace data, or playback state. Every event carries Game/Test mode; full Program source, Notebook text, machine identity, and unrelated personal data are excluded.
- `src/playtest/playtest_feedback_overlay.gd` owns concise skippable chapter and Demo forms. `src/ui/level_completion_overlay.gd` optionally embeds the three per-level ratings and bounded note after authoritative completion. Normal player launches enable these surfaces; script, headless, and capture launches suppress them unless a focused test explicitly opts in.
...
- `tests/test_playtest_data.gd` verifies unique sessions, append durability, malformed-tail recovery, Test-mode attribution, counters, bounded text, summary derivation, clean shutdown, and export schema/privacy. `tests/test_playtest_feedback_ui.gd` verifies localized level/chapter/Demo submission, skip paths, and questionnaire-disabled non-blocking behavior.
