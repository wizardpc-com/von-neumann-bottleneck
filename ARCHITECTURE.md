...
## Default eight-task mainline

`src/demo/demo_menu.tscn` is the project entry. `demo_workbench.gd` presents a fixed circuit/program/data workspace and immediate, nonmodal completion. `demo_foundations.gd` verifies visible standard-module circuits through the existing PrologueSimulator; `demo_performance.gd` binds explicit standard system/locality supplies, unchanged engines, workload checks and comparison identity. `demo_circuit_board.gd` uses GraphEdit and the existing procedural module renderer. `demo_progress.gd` owns independent recoverable Game saves and replays accepted designs. See [current status](docs/status/demo-redesign.md) and [ADR 0019](docs/decisions/0019-eight-task-mainline-and-standard-modules.md).

The following flows describe the retained optional workshop and legacy lab scenes. Their old progression/prediction/Apply/review gates do not govern the new mainline. Workshop provenance and all simulation boundaries remain in force.
