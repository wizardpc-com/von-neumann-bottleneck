...
This file is the high-level map. Detailed simulation behavior lives in [`docs/architecture/simulation.md`](docs/architecture/simulation.md); current slice facts live in [`docs/status/cpu-building-prologue.md`](docs/status/cpu-building-prologue.md), [`docs/status/chapter-1-waiting-for-data.md`](docs/status/chapter-1-waiting-for-data.md), and [`docs/status/chapter-2-reducing-data-movement.md`](docs/status/chapter-2-reducing-data-movement.md).
...
- `src/ui/floating_instrument_panel.gd` provides embedded movable, resizable, independently closable windows and allows a host-owned compact action to replace generic title-strip minimization. Hardware Mission uses that action for its readable lower-left compact state; Chapter 1 and Chapter 2 Mission disable minimization, while other tools may retain it. Launcher buttons use close-equivalent toggling, so hiding and reopening preserves the panel's current geometry.
- `src/ui/ui_typography.gd` owns the shared title, subtitle, body, caption, window-title, and compact-control dimensions used across chapter headers, Mission content, completion summaries, and the terminology handbook. `src/ui/terminology_handbook.gd` presents its cross-chapter catalog as a bounded topic → directory → term tree while retaining category filtering and search.
