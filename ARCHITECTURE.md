...
The original chapters share `InstrumentTheme`, `TechnicalBackdrop` and bundled Noto Sans SC typography. `SignalLevelButton` preserves the existing CheckButton input contract while drawing compact low/high symbols. Palette thumbnails reuse the canvas schematic and module renderers; placement ghosts retain the full GraphNode geometry. `TerminologyHandbook.LESSON_TERMS` derives subject availability from existing original progression, including each playable Mission's required specifications. It adds no persistent unlock authority or save fields. See [visual and learning polish](docs/status/visual-learning-polish.md).
