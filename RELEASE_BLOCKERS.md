...
Final source recheck and current release-binary probe pass. The previous candidate
mouse attempt returned `noWindowsAvailable`; current candidate mouse checks remain pending. Do not transfer source/older package
...
| Candidate mouse navigation | **Final recheck pending** | Previous content only: Actual candidate binary/PCK in an isolated QA copy: mouse tree pan/zoom/entry, palette drag, wires/delete/undo, Tutorial completion and restart/Continue; [evidence](docs/verification/20260913-spatial-candidate/README.md). Earlier noWindowsAvailable is historical. |
