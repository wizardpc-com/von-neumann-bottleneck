...
手册按当前任务推荐少量概念，后续知识逐步开放。设置中的“减少动效”可减少窗口淡入和导线回放运动，运行结果与计时不变。

第四章：选择字段分组、排列和分块；后续任务引入实际复制成本与有限临时区。
同样正确的输出可以有不同有效布局。准备/查询/输出时间、读写流量和临时峰值分别显示。
点击运行结果的批次事件，可以看到该批真实地址；尾批保留余数记录。

首页顶部可直接切换中英文；设置也可切换语言、调整音效和显示、导出本地诊断。恢复默认设置不会改动进度、方案或隐私授权。
...
回退旧程序前先备份整套文件。当前版本遇到较新格式或最低写入版本不兼容时会阻止覆盖；不要用旧备份手动替换新版主存档来绕过保护。
