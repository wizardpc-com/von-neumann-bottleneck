...
- Run History should present the most decision-relevant controlled pair as Before → After, naming the changed item and total/CPU-wait deltas before listing raw run facts.
- The capstone introduces no new mechanism or hint. It first locks changes long enough to capture the given baseline, then accepts multiple hardware/software solutions and remains runnable after completion so optimization can continue voluntarily.
