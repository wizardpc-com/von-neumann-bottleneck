...
- In Hardware Foundations, Mission and Test Bench behave as desktop-style windows over the circuit board: they may coexist, move, resize, close, and restore without changing simulation. Mission's compact action keeps readable content at the lower-left with Test Bench width instead of producing a title strip; taskbar actions toggle close/open while preserving the current geometry.
- `Esc` resolves the nearest active layer first: handbook, menu, or editor gesture; then level → map → chapter selection; then the chapter-selection Options overlay. It must never perform two levels of this chain from one press, and a scene-changing press must be consumed before the old scene is removed.
