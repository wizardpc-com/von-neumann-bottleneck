...
The first-playtest inventory audit keeps the palette permissive while making the initial desktop intentional. Tutorial pre-places only `A`, `NOT`, and `LAMP`; Half Adder pre-places only its two inputs, two AND gates, one OR gate, one NOT gate, and two outputs. Full Adder, ALU, Latch, Register, RAM, CPU, and LOAD/STORE retain only components that participate in their registered reference topology. Optional or alternative parts remain available from Components and are not presented as mandatory starting pieces.
