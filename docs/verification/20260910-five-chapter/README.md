...
## M5 frozen candidate delivery

Final content commit **435d10f4cd8888c8cd44f9402a2f4cefb5d5e4a8**, Godot **4.7.1.stable.official.a13da4feb**. Both candidates built from one Git archive by `scripts/build-free-candidate.py`, output `build/free-alpha-20260910T171313Z-435d10f4/`. Import, license extraction, both exports and ZIP CRC checks pass. Final export logs contain no ERROR/WARNING; numeric Windows file metadata fixed the intermediate version-format warnings. Per-file hashes reverified after export; exact archive hashes are in [candidate-manifest.json](candidate-manifest.json). No package is publicly uploaded.

- macOS ZIP: **77,865,549 bytes**; SHA-256 `af7417998b910d51771ccbf3dd8607c552c63f2df05eeb18737e443a561a4de2`.
- Windows ZIP: **55,397,134 bytes**; SHA-256 `a4bcdba94415720d94e76c6975d96cbe4820d9d07541af43ae8039c02c6664d3`.
- Mac binary contains x86_64 and arm64; original exported bundle passes `codesign --verify --deep --strict` (ad-hoc, not notarized). Windows file is PE32+ GUI x86_64; **no Windows execution claimed**.

Actual release-binary probe, `.godot/package-qa/774bee7090a9`, passes: isolated user path, `free_candidate` feature, Game only even with Test arguments, capture arguments disabled, 40 tasks, remote endpoint empty/off, test and server files absent. The probe used a QA-only external `override.cfg` next to the executable, as loaded by [Godot 4.7.1 ProjectSettings](https://github.com/godotengine/godot/blob/4.7.1-stable/core/config/project_settings.cpp#L689). An additional sandbox denied both reading and writing the original player directory. The original binary and PCK hashes remained unchanged through this probe. Reproduce with `python3 scripts/verify-mac-candidate.py --app /path/to/candidate.app`.

The release app's actual hub and task tree were also opened through CUA. Multiple same-ID local builds caused `noWindowsAvailable` and long tool waits. For native control only, the QA copy was renamed `VNB Candidate QA.app`, assigned unique ID `com.wizardpc.vnb.qa774bee7090a9`, and ad-hoc resigned; **the delivered ZIP/app were not changed**. This copy retains the exported PCK and isolated userdata. CUA successfully clicked the normal task-tree entry and observed the fresh prerequisite-gated tree. Further controls encountered user-interaction guards, so the app was left available for the user rather than fighting their input. This is **package startup/tree evidence, not a full package walkthrough**.

The explicit 1280×800-pixel diagnostic launch appears as a very small 640×400-point window on Retina and is uncomfortable; normal fullscreen is readable and matches the earlier 1224×768-point source-native inspection. No claim is made for comfort below 960 logical points. Minimum-size enforcement, thorough app-switch cancellation, the final BackSpace fix's physical-key repeat, another-Mac installation, Windows native play and external novice testing remain the convergence queue. Source tests already protect BackSpace text focus and undo; they do not replace its pending native check.

### Scope at handoff

M0 baseline protected; M1 checkpoint and M2 six-task native play completed; M3 automatic tree and actual UI fixes delivered; M4 local feedback/report integration and counter calibration completed; M5 reproducible free candidates and distribution/Steam preparation delivered with the explicit native/platform limits above. The original player profile was neither migrated in place nor reset for QA. Uploaded root plan/prompt remain untouched. Public release, hosted receiver, payments and identity submission remain outside this handoff.
