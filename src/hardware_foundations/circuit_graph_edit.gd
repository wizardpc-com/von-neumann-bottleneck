		...
		if mouse_event.pressed:
			...
			if mouse_event.shift_pressed and not pressed_port.is_empty() and not bool(pressed_port.get("is_output", true)):
				...
				if not existing.is_empty():
					...
					accept_event()
					return
			if mouse_event.shift_pressed and pressed_port.is_empty() and _node_at(mouse_event.position).is_empty():
...
func _apply_selection_rectangle() -> void:
	...
	for child: Node in get_children():
		...
		if not selection_rect.intersects(Rect2(node.position, node.size), true):
			continue
