...
func record_pass(id: String, board: Dictionary, program: String) -> bool:
	...
	if not Catalog.evaluate(id, board, program).passed: return false
...
func game_snapshot() -> Dictionary:
	return {"schema_version": 1, "solutions": game_solutions.duplicate(true), "drafts": game_drafts.duplicate(true)}
